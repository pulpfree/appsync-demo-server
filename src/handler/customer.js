...
CustomerHandler.prototype.findOne = (args) => {
  let cust
  try {
    cust = Customer.findById(args.id)
  } catch (e) {
    throw new Error(e)
  }
  ...
  try {
    cust.address = Address.findOne({ customerID: cust._id, associate: 'customer' })
  } catch (e) {
    throw new Error(e)
  }
...
CustomerHandler.prototype.search = (args) => {
  const { field } = args
  ...
  let res
  try {
    res = Customer.aggregate(pipeline)
  } catch (e) {
    throw new Error(e)
  }
